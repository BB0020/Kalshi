...
COINGECKO_MARKET_CHART = "https://api.coingecko.com/api/v3/coins/bitcoin/market_chart?vs_currency=usd&days=1&interval=minutely"

def fetch_btc_price_kraken_index():
    """Use Kraken’s BTC/USD index (CF Benchmarks-powered under the hood)."""
    r = requests.get(KRAKEN_TICKER, timeout=8)
    r.raise_for_status()
    data = r.json()
    result = data["result"]
    # Kraken pairs usually come as keys like "XXBTZUSD"
    key = list(result.keys())[0]
    last_trade = result[key]["c"][0]  # "c" = last trade price, index 0 is price
    return float(last_trade)

def fetch_btc_price_coingecko():
    """Fallback BTC price source."""
...
def fetch_current_btc_price():
    """
    DEFAULT → Kraken index (CF-derived)
    FALLBACK → CoinGecko
    """
    try:
        ...
        try:
            fallback = fetch_btc_price_coingecko()
            return fallback, "CoinGecko (fallback)"
        except:
            return None, "Error fetching price"

def fetch_minute_prices_last_n(n_minutes=240):
    """Fetch recent minute-level BTC prices (CoinGecko)."""
...
def realized_log_sigma_per_minute(prices_series):
    """Compute per-minute log-return volatility."""
...
def prob_price_above_strike_log_normal(S0, K, sigma_log_per_minute, minutes_remaining):
    """Probability BTC ends above strike using log-normal model."""
...
st.caption("iPhone-optimized • CF-derived price (Kraken) • Free APIs only")
...
with col1:
    strike = st.number_input(
        ...
        value=96500.0, step=50.0, format="%.2f"
    )
    minutes_for_vol = st.slider(
        "Use last N minutes to estimate volatility:",
        min_value=30, max_value=1440, value=240, step=30
    )
...
with col2:
    kalshi_price = st.number_input(
        "Kalshi YES price (0 to 1)",
        min_value=0.0, max_value=1.0, value=0.40, step=0.01, format="%.4f"
    )

# Time to hour end (UTC)
now = datetime.utcnow()
hour_end = (now.replace(minute=0, second=0, microsecond=0) + pd.Timedelta(hours=1)).to_pydatetime()
...
if btc_price is None:
    ...
    status_box.info(f"BTC from {source}: ${btc_price:,.0f} • UTC {now.strftime('%H:%M')}")

# Fetch minute data for volatility
sigma = None
try:
...
# Fallback sigma if needed
if sigma is None or sigma == 0:
    sigma = 0.0008  # ≈2% hourly vol approximation
...
with cB:
    ...
    st.metric("Kalshi implied", human_pct(kalshi_price))
...
st.write(f"- YES market price: **{kalshi_price:.3f}**")
st.write(f"- EV(YES): **{ev:.4f}**")
st.write(f"- EV(NO): **{no_ev:.4f}**")
st.write(f"- ROI if YES wins: **{(1-kalshi_price)/kalshi_price:.2f}×**")

st.markdown("---")
st.caption("Tip: In Safari → Share → Add to Home Screen to use this like an app.")
